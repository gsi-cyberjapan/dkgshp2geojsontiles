...
my $shpname = '-RvrCL-';
#取り出したshpの置き場所
my $shpdir = 'D://rvrclshp/';
#geojsonのpropertiesへ出力するshpの属性項目の設定
my @outproperty = ('class','rID','lfSpanFr','lfSpanTo','tmpFlg','orgGILvl','ftCode','admCode','devDate','type','name','rivCtg','admOfcRiv','rivCode');
#geojsonのpropertiesへ出力するshpの属性項目が文字列か数値かの判別（文字列：0、数値：1）
my %outproperty_num = ('class' => 0,'rID' => 0,'lfSpanFr' => 0,'lfSpanTo' => 0,'tmpFlg' => 1,'orgGILvl' => 0,'ftCode' => 0,'admCode' => 0,'devDate' => 0,'type' => 0,'name' => 0,'rivCtg' => 0,'admOfcRiv' => 0,'rivCode' => 0);
###########################################################################################################
