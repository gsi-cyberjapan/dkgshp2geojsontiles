...
sub wantedshp{
  $karidir = $File::Find::dir, '/';    #カレントディレクトリ
  @karidira = split(/\//,$karidir);
  $kariList = $_;          #ファイル名
 #shpを検索
  if($kariList =~ /\.shp/ ){
   my @d= split(/\./,$kariList);
...
foreach my $key (keys(%tlcnt)){
 @k= split(/\//,$key);
 $outdir = $gjndir.$karidira[@karidira-1]."/". @k[0]."/".@k[1];
 $outjson = $gjndir.$karidira[@karidira-1]."/".$key.".geojson";
 mkpath($outdir);
...
close(FOUT);
 ...
 if ($tlcntall{$key}) {
   $tlcntall{$key}=$tlcntall{$key}.",".$outjson;
 } else {
   $tlcntall{$key}=$outjson;
 }
...
#データの読み込み
 open(FH, @fileList[$i]);
 my @status_FH = stat(FH);
 my $size_of_FH = $status_FH[7];
 read(FH, $s_data, $size_of_FH);
 close(FH);
#書き出し
 chomp($s_data);
 print FOUT $s_data;
